...
/**
 * 拼房间附件可访问 URL（供跨机可达的公开基址使用）。
 * @param {string} base http(s)://host:port（容忍尾部 /）
 */
export function chatFileUrl(base, id, roomId) {
  const b = String(base || '').trim().replace(/\/+$/, '');
  return b + '/chat-file/' + encodeURIComponent(String(id || '')) + '?room=' + encodeURIComponent(String(roomId || ''));
}
