...
import { formatFileMsg } from "./file-msg.js";
...
export function apply(ctx, config = {}) {
  ...
  const uploadDisposer = ctx.tools.register(defineTool({
    ...
    parameters: Schema.object({
      filePath: Schema.string().description("本地文件绝对路径"),
      roomId: Schema.string().description("房间标识（必填）")
    }),
    output: textOutput(),
  ...
  const fetchDisposer = ctx.tools.register(defineTool({
    ...
    parameters: Schema.object({
      url: Schema.string().description("附件 url（http/https）"),
      saveDir: Schema.string().description("可选保存目录（默认 ~/.dsh/dsh-chat/downloads）")
    }),
    output: textOutput(),
